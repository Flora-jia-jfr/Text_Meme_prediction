...
def train(train_loader, hyp_params, model, bert, tokenizer, feature_extractor, optimizer, criterion, epoch):
    ...
    for i_batch, data_batch in enumerate(train_loader):
        ...
        if i_batch % hyp_params.log_interval == 0 and i_batch > 0:
            if hyp_params.dataset == 'multi_category_memotion_dataset':
                train_acc, train_f1, train_f1_macro, train_precision, train_recall = metrics(preds, targets,
            ...
            msg = 'Epoch {:2d} | Batch {:3d}/{:3d} | Time/Batch(ms) {:5.2f} | Train Loss {:5.4f} | Acc {:5.4f} | micro-score {:5.4f} | macro-score {:5.4f} | precision {:5.4f} | recall {:5.4f}'.format(
                epoch, i_batch, num_batches, elapsed_time * 1000 / hyp_params.log_interval, avg_loss, train_acc,
                train_f1, train_f1_macro, train_precision, train_recall)
            print(msg)
            write_log(msg)
            proc_loss, proc_size = 0, 0
            start_time = time.time()
...
def evaluate(valid_loader, hyp_params, model, bert, tokenizer, feature_extractor, criterion, train=False,
             train_loader=None):
    model.eval()
