...
logging.basicConfig(filename= "./pretrained_models/multi_category_meme_model_result.log",
                    level = logging.INFO,
                    format=LOG_FORMAT)
...
def metrics(results, truths, dataset=""):
    ...
    precision = precision_score(truth, preds, average='None' if dataset == "" else "macro")
    recall = recall_score(truth, preds, average='None' if dataset == "" else "macro")
