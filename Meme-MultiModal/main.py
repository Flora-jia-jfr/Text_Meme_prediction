...
def train_model(settings, hyp_params, train_loader, val_loader, test_loader=None):
    ...
    for epoch in range(1, hyp_params.num_epochs + 1):
        ...
        train_acc, train_f1, train_f1_macro, train_precision, train_recall = metrics(train_results, train_truths,
                                                                                     dataset="multi_category_memotion_dataset" if hyp_params.dataset == 'multi_category_memotion_dataset' else "")
        val_acc, val_f1, val_f1_macro, val_precision, val_recall = metrics(results, truths,
                                                                           dataset="multi_category_memotion_dataset" if hyp_params.dataset == 'multi_category_memotion_dataset' else "")
        print("-" * 100)
        msg = 'Epoch {:2d} | Time {:5.4f} sec | Train Loss {:5.4f} Acc {:5.4f} micro-score {:5.4f} macro-score {:5.4f} | Valid Loss {:5.4f} Acc {:5.4f} mirco-score {:5.4f}  macro-score {:5.4f}'.format(
            epoch, duration, train_loss, train_acc, train_f1, train_f1_macro, val_loss, val_acc, val_f1, val_f1_macro)
        print(msg)
        print("-" * 100)
        msg1 = 'Epoch {:2d} | Time {:5.4f} sec | Train Loss {:5.4f} | Train Acc {:5.4f} | micro-score {:5.4f} | macro-score {:5.4f} | precision {:5.4f} | recall {:5.4f}'.format(
            epoch, duration, train_loss, train_acc, train_f1, train_f1_macro, train_precision, train_recall)
        msg2 = 'Epoch {:2d} | Time {:5.4f} sec | Valid Loss {:5.4f} | Valid Acc {:5.4f} | mirco-score {:5.4f} | macro-score {:5.4f} | precision {:5.4f} | recall {:5.4f}'.format(
            epoch, duration, val_loss, val_acc, val_f1, val_f1_macro, val_precision, val_recall)
        write_log(msg1)
        write_log(msg2)
    ...
    train_acc, train_f1, train_f1_macro, train_precision, train_recall = metrics(results, truths,
                                                                                 dataset="multi_category_memotion_dataset" if hyp_params.dataset == 'multi_category_memotion_dataset' else "")
    msg = "\n\nTrain Loss {:5.4f} | Train Acc {:5.4f} | Train f1-score {:5.4f} | Train f1-score-macro {:5.4f} | Train precision {:5.4f} | Train recall {:5.4f}".format(
        train_loss, train_acc, train_f1, train_f1_macro, train_precision, train_recall)
    print(msg)
    write_log(msg)
    ...
    test_acc, test_f1, test_f1_macro, test_precision, test_recall = metrics(results, truths,
                                                                            dataset="multi_category_memotion_dataset" if hyp_params.dataset == 'multi_category_memotion_dataset' else "")
    msg = "\n\nTest Loss {:5.4f} | Test Acc {:5.4f} | Test f1-score {:5.4f} | Test f1-score-macro {:5.4f} | Test precision {:5.4f} | Test recall {:5.4f}".format(
        test_loss, test_acc, test_f1, test_f1_macro, test_precision, test_recall)
    print(msg)
    write_log(msg)
    sys.stdout.flush()
